      ...
      <div>
        ...
        >
          ...
          <div className="relative z-10 container mx-auto px-6 max-w-7xl">
            ...
            <nav className="mb-12" aria-label="Project category filter">
              <div className="flex flex-wrap justify-center gap-3" role="tablist">
                {categories.map((category) => (
                  <button
                    key={category.id}
                    onClick={() => setActiveCategory(category.id)}
                    className={`px-5 py-2.5 rounded-full font-medium text-sm tracking-wide transition-all duration-300 ${activeCategory === category.id
                        ? "bg-white text-black"
            ...
            {/* Bottom CTA */}
            <footer className="text-center mt-20 pt-12 border-t border-neutral-800/50">
              <p
